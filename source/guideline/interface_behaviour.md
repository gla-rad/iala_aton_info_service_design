...
All interface descriptions below only have the information necessary for the business logic of this service. All definitions and descriptions that are directly derived from the SECOM standard are left to be read from the SECOM standard and the API documentation template in the Annex A.
...
The SECOM maximum upload size limit (i.e. 350KB), does not apply to the Get operation. As long as the HTTP server allows it, the data transfer of any size is allowed.
...
| page | Integer | 0..1 | The number of the page for retrieving paged results. If not provided, the default value of this parameter is set to one (1). |
| pageSize | Integer | 0..1 | The size of each page in a paged result. If not provided, the default value of this parameter is not set and the returned S-125 datasets are not paged. |
| atonDetails (Optional)| Boolean | 0..1 | Requests that the service returns summary information for datasets which include the latest AtoN design state details (not just the default AtoN status indication features), if available. |
...
| atonDetails (Optional)| Boolean | 0..1 | Requests that the service responds with datasets which include the latest AtoN design state details (not just the default AtoN status indication features), if available. |
